    ...
    return __awaiter$1(this, void 0, void 0, function* () {
      try {
        return yield _request(this.fetch, "POST", `${this.url}/recover`, {
          body: { email: email2, gotrue_meta_security: { captcha_token: options.captchaToken } },
          headers: this.headers,
          redirectTo: options.redirectTo
        });
      } catch (error) {
...
let Partida$1 = class Partida2 {
  ...
  // leer todos
  static async getAll() {
    const { data: partidas2, error } = await supabase.from("partidas").select("*");
    if (error) {
      throw new Error(error.message);
    }
    return partidas2.map(({ id, created_at, puntos, tiempo, usuario_id }) => {
      return new Partida2(id, created_at, puntos, tiempo, usuario_id);
    });
  }
  // leer registro por id (método static que se puede leer desde la clase sin necesidad de crear una instancia)
  static async getById(id) {
    const { data: partidas2, error } = await supabase.from("partidas").select("*").eq("id", id).single();
    if (error) {
      throw new Error(error.message);
    }
    return new Partida2(partidas2.id, partidas2.created_at, partidas2.puntos, partidas2.tiempo, partidas2.usuario_id);
  }
  static async getByUserId(id) {
    const { data: partidas2, error } = await supabase.from("partidas").select("*").eq("usuario_id", id).single();
    if (error) {
      throw new Error(error.message);
    }
    return new Partida2(partidas2.id, partidas2.created_at, partidas2.puntos, partidas2.tiempo, partidas2.usuario_id);
  ...
  static async create(partidaData) {
    const { error } = await supabase.from("partidas").insert(partidaData).select();
    if (error) {
      throw new Error(error.message);
    }
    return true;
...
class Perfil {
  ...
  static async getByUserId(id) {
    const { data: perfil, error } = await supabase.from("perfiles").select("*").eq("usuario_id", id).single();
    if (error) {
      throw new Error(error.message);
    }
    return new Perfil(perfil.id, perfil.created_at, perfil.nombre, perfil.apellidos, perfil.avatar, perfil.nick, perfil.usuario_id, perfil.estado, perfil.rol);
...
let User$1 = class User2 {
  // Mapping de propiedades de la tabla perfiles
  constructor(id = null, email2 = null, password = null) {
    this.id = id;
    this.email = email2;
...
const partidas = {
  ...
  `,
  script: async () => {
    const partidas2 = await Partida$1.getAll();
    let filas = "";
...
class PokemonBD {
  ...
  static async getById(id) {
    const { data: pokemons, error } = await supabase.from("pokemons").select("*").eq("id", id).single();
    if (error) {
      throw new Error(error.message);
    }
  ...
  static async añadir(pokemonData) {
    const { error } = await supabase.from("pokemons").insert(pokemonData).select();
    if (error) {
      throw new Error(error.message);
    }
    return true;
  ...
  // actualizar
  async update() {
    const { error } = await supabase.from("pokemons").update({
      nombre: this.nombre,
      tipo: this.tipo,
      imagen: this.imagen,
...
const pokemonsTabla = {
  template: `<table>
    ...
    <tbody>`,
  script: async () => {
    const partidas2 = await PokemonBD.getAll();
    let filas = "";
    partidas2.forEach((element) => {
      filas += `
          <tr>
...
<div class="modal fade" id="registroModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="false">
  ...
  script: async () => {
    try {
      const usuario = await User$1.getUser();
      console.log(usuario);
    ...
    document.querySelector("#registroPerfil").addEventListener("click", async (e) => {
      ...
      try {
        ...
        const usuario = await User$1.create(usuarioAcrear);
        const perfilAcrear = {
          nick: nickUsuario,
    ...
    document.querySelector("#login").addEventListener("click", async (e) => {
      ...
      try {
        let usuarioLoggeado = await User$1.login(usuarioAloggear);
        document.querySelector("#nickUsuario").innerHTML = `${(await Perfil.getByUserId(usuarioLoggeado.id)).nick}`;
        document.querySelector("#opcionesMenu").innerHTML = `
          <li><a class="dropdown-item" id="logout">logout</a></li>
    `;
        document.querySelector("#logout").addEventListener("click", async (e2) => {
          await User$1.logout();
          document.querySelector("#nickUsuario").innerHTML = ``;
          location.reload();
        });
      ...
      if (typeof args[0] === "object" && !isElement2(args[0])) {
        ...
        ["title", "html", "icon"].forEach((name2, index) => {
          ...
          if (typeof arg === "string" || isElement2(arg)) {
            ...
            error(`Unexpected type of ${name2}! Expected "string" or "Element", got ${typeof arg}`);
          }
        });
      }
      return params;
...
class Pokemon {
  constructor() {
    __publicField(this, "cargaDatosPokemon", async () => {
    ...
    __publicField(this, "generaDatosAleatorios", async () => {
    ...
    __publicField(this, "insertaPokemon", async () => {
    ...
    __publicField(this, "mataPokemon", async (id) => {
    ...
    __publicField(this, "observadores", async () => {
                ...
                if (result.isConfirmed) {
                  try {
                    ...
                    if (usuario) {
                      ...
                      sweetalert2_allExports.fire("Guardado !", "", "success");
                    }
                  } catch (error) {
                    sweetalert2_allExports.fire({
                      title: "Error !",
                  ...
                  sweetalert2_allExports.fire("Changes are not saved", "", "info");
                }
              });
            }
            this.mataPokemon(idDiv);
...
const pokemonSupabase = {
  ...
  script: async () => {
    ...
    for (let i = 1; i <= 20; i++) {
      ...
      for (let i2 = 0; i2 < pokemonsBD.length; i2++) {
        let pokedexBaseDeDatos = parseInt(await pokemonsBD[i2].pokedex);
    ...
    async function iniciarPartida() {
      ...
      async function insertarPokemons(pokemonsApintar) {
        ...
        const intervalo = setInterval(() => {
          if (document.querySelector("#puntos").textContent == 2e3) {
            clearInterval(intervalo);
          } else {
            contador2++;
            tiempoElemento.textContent = contador2;
            if (contador2 === 10) {
              clearInterval(intervalo);
              let puntosJugador = document.querySelector("#puntos").textContent;
              sweetalert2_allExports.fire({
                ...
                if (result.isConfirmed) {
                  try {
                    ...
                    if (usuario) {
                      ...
                      console.log(partida);
                      await Partida$1.create(partida);
                      sweetalert2_allExports.fire("Guardado !", "", "success");
                    }
                  } catch (error) {
                    sweetalert2_allExports.fire({
                  ...
                  sweetalert2_allExports.fire("La partida no se ha guardada", "", "info");
                }
              });
            }
          }
    ...
    document.querySelector("#guardarPartida").addEventListener("click", async (e) => {
      try {
        const usuarioLoggeado = await User$1.getUser();
        let partida = {
